...
const SetBuilderContext = createContext<
...
export const SetBuilderProvider: React.FC<SetBuilderProviderProps> = ({
  ...
  const handleEqSelect = (item: Armor | Weapon | Charm) => {
    ...
    const newSlottedEq = {
      equipment: item,
      accessories: Array(item.slots.length).fill(null),
    };
  ...
  const startSearch = () => {
    ...
    const hasAccessories = Object.values(currentEquipmentSet).some(
      (eq) =>
        eq && eq.accessories.some((acc: Accessory | null) => acc !== null),
    );
